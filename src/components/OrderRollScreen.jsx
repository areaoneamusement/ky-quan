...
export default function OrderRollScreen() {
  ...
  const { players, orderRolls, orderRollQueue, dice, diceRolled, log } = state
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '24px' }}>
          {players.map(p => {
            ...
            const isCurrent = p.id === currentRollerId
            return (
              <div key={p.id} style={{
                display: 'flex', alignItems: 'center', gap: '10px',
                padding: '8px 12px', borderRadius: '8px', marginBottom: '6px',
                background: isCurrent ? 'rgba(63,185,80,0.08)' : 'rgba(255,255,255,0.03)',
                border: '1px solid ' + (isCurrent ? 'rgba(63,185,80,0.25)' : '#30363d'),
              }}>
                <div style={{
